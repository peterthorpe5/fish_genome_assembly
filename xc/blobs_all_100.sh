...
cd /storage/home/users/pjt6/fish_genomes/xc/split_fasta_files

fasta=xc_CLC_sp_scaff._10?.fasta
...
do
    ...
    CMD="blobtools create -i ${f}  --type spades -t ${f}_blast -o XC_V1.blobplots_${f} --nodes /shelf/public/blastntnr/blastDatabases/nodes.dmp --names /shelf/public/blastntnr/blastDatabases/names.dmp"
    echo ${CMD}
    eval ${CMD}
    mkdir ${f}_allfinal.fa.blobplots
    cp XC_V1.blobplots_${f}.blobDB.json ./${f}_allfinal.fa.blobplots
    echo "blobtools view for ${f}"
    blobtools view -i  XC_V1.blobplots_${f}.blobDB.json -o ${f}_allfinal.fa.blobplots/
    echo "blobtools plot for ${f}"

    blobtools plot -i XC_V1.blobplots_${f}.blobDB.json -o ${f}_allfinal.fa.blobplots/ 
    ...
    #python ~/public_scripts/get_sequences_i_want_from_fasta_command_line_not_wanted_file.py scaffold.fasta ./${f}_allfinal.fa.blobplots/bad_contigs.out 10 xc_contanim_filtered_v1.fasta
